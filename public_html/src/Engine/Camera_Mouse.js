...
Camera.prototype.isMouseInViewport = function (dcX, dcY) {
    return ((dcX >= parseInt(this.mViewport[0])) && (dcX < (parseInt(this.mViewport[0]) + parseInt(this.mViewport[2]))) &&  // viewport[2] is width
            (dcY >= parseInt(this.mViewport[1])) && (dcY < (parseInt(this.mViewport[1]) + parseInt(this.mViewport[3]))));   // viewport[3] is height
};
